<x-admin-layout>
    <div class="mb-8">
        ...
        <h1 class="mt-2 text-2xl font-bold text-gray-900">Novo Artista</h1>
    </div>

    <div class="mx-auto max-w-2xl">
        <form method="POST" action="{{ route('admin.settings.artists.store') }}" class="rounded-lg bg-white p-6 shadow">
            @csrf
            <div class="space-y-6">
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="slug" id="slug" value="{{ old('slug') }}" placeholder="Gerado automaticamente" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                ...
                <div>
                    ...
                    <input type="text" name="discogs_id" id="discogs_id" value="{{ old('discogs_id') }}" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                ...
                <div>
                    ...
                    <input type="url" name="discogs_url" id="discogs_url" value="{{ old('discogs_url') }}" placeholder="https://www.discogs.com/artist/..." class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    @error('discogs_url')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label for="profile" class="block text-sm font-medium text-gray-700">Perfil/Biografia</label>
                    <textarea name="profile" id="profile" rows="4" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('profile') }}</textarea>
                </div>
