<x-admin-layout>
    <div class="mb-8">
        ...
        <h1 class="mt-2 text-2xl font-bold text-gray-900">Novo Peso</h1>
    </div>

    <div class="mx-auto max-w-lg">
        <form method="POST" action="{{ route('admin.settings.weights.store') }}" class="rounded-lg bg-white p-6 shadow">
            @csrf
            <div class="space-y-6">
                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700">Nome *</label>
                    <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="Ex: LP Simples, LP Duplo" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    @error('name')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label for="value" class="block text-sm font-medium text-gray-700">Valor *</label>
                        <input type="number" name="value" id="value" value="{{ old('value') }}" required min="1" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <div>
                        ...
                        <select name="unit" id="unit" required class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            <option value="g" {{ old('unit', 'g') === 'g' ? 'selected' : '' }}>Gramas (g)</option>
