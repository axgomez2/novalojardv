<x-admin-layout>
    <div class="mb-8">
        ...
        <h1 class="mt-2 text-2xl font-bold text-gray-900">
            Novo Pagamento Recorrente - {{ $type === 'payable' ? 'Despesa' : 'Receita' }}
        </h1>
    </div>

    <form method="POST" action="{{ route('admin.financial.recurring.store') }}" class="space-y-6">
        @csrf
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <select name="frequency" id="frequency" required
                        ...
                        <option value="biweekly" {{ old('frequency') == 'biweekly' ? 'selected' : '' }}>Quinzenal</option>
                        <option value="monthly" {{ old('frequency', 'monthly') == 'monthly' ? 'selected' : '' }}>Mensal</option>
                        <option value="bimonthly" {{ old('frequency') == 'bimonthly' ? 'selected' : '' }}>Bimestral</option>
                        <option value="quarterly" {{ old('frequency') == 'quarterly' ? 'selected' : '' }}>Trimestral</option>
                        <option value="semiannual" {{ old('frequency') == 'semiannual' ? 'selected' : '' }}>Semestral</option>
                        <option value="annual" {{ old('frequency') == 'annual' ? 'selected' : '' }}>Anual</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="day_of_month" id="day_of_month" value="{{ old('day_of_month') }}" min="1" max="31"
                           class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    <p class="mt-1 text-xs text-gray-500">Para frequências mensais ou superiores</p>
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="end_date" id="end_date" value="{{ old('end_date') }}"
                           class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    <p class="mt-1 text-xs text-gray-500">Deixe vazio para recorrência indefinida</p>
                </div>
                ...
                <div>
                    ...
                    <select name="payment_method" id="payment_method"
                        ...
                        <option value="debito_automatico" {{ old('payment_method') == 'debito_automatico' ? 'selected' : '' }}>Débito Automático</option>
                    </select>
                </div>

                <div>
                    <label for="days_before_notify" class="block text-sm font-medium text-gray-700">Dias para Notificar</label>
                    <input type="number" name="days_before_notify" id="days_before_notify" value="{{ old('days_before_notify', 5) }}" min="0" max="30"
                           class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    <p class="mt-1 text-xs text-gray-500">Quantos dias antes do vencimento notificar</p>
                </div>
                ...
                <div class="flex items-center gap-6">
                    <div class="flex items-center">
                        <input type="checkbox" name="auto_generate" id="auto_generate" value="1" {{ old('auto_generate', true) ? 'checked' : '' }}
